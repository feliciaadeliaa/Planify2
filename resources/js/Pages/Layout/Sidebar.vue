...
// Define reactive variables for storing recent projects and tasks
const recentProjects = ref([]);
const recentTasks = ref([]);

// Fetch the recent projects and tasks when the component is mounted
const fetchRecentItems = async () => {
  try {
    const response = await axios.get(route('recent.items', usePage().props.auth.id));
      ...
      <nav class="-mx-3 space-y-6">
        ...
        :href="route('goal.index')"
          ...
          >
            ...
            <span class="mx-2 text-sm font-medium">Calender</span>
          </a>
        ...
        <a
         v-for="(item) in recentProjects" :key="item.id"
            ...
            :href="route('project.detail', item.id)"
          >
          <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
            ...
            <span class="mx-2 text-sm font-medium">{{ item.project_title }}</span>
          </a>

        <div class="space-y-3">
          <label class="px-3 text-xs text-gray-500 uppercase dark:text-gray-400"
            >Recent To-Do's</label
          >
        </div>
